...
class Problem:
    ...
    def __init__(self, power_symb = "^", auto_generate=True, enable_frac=True, enable_pow=True):
        self.power_symb = power_symb
        self.root = Node(self.power_symb)
        self.operator_set = ["+", "-", "*", "/"]
        self.operator_weight = [8, 8, 4, 2]
        if enable_pow:
            self.operator_set.append("^")
            self.operator_weight.append(2)
    ...
    def generate(self):
        while True:
            ...
            if h in self.hashing_set:
                continue
            if not self.enable_frac and self.root.number.denominator != 1:
                continue
            else:
